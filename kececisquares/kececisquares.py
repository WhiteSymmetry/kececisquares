...
"""
sağ sol hatalı çıkıntı
def kececi_binomial_trapezoid(binomial_triangle_data, trapezoid_height, start_row_index, alignment_type):
    Pascal üçgeni üzerinde yamuk şeklinde bir bölge seçer.
    Parameters:
    - binomial_triangle_data: Pascal üçgeni verisi
    - trapezoid_height: Yamuğun yüksekliği (satır sayısı)
    - start_row_index: Yamuğun başlayacağı satır indeksi
    - alignment_type: Yamuğun hizalanma türü ("left", "right", veya "center")
    Returns:
    - series_data: Yamuk bölgesindeki değerler
    - total_value: Yamuk bölgesindeki değerlerin toplamı
    - selected_indices_info: Seçilen indekslerin bilgileri
    ...
    for row_offset in range(trapezoid_height):
        ...
        if alignment_type == "left":
            # Sol hizalı yamuk: Her satırın ilk `trapezoid_height + row_offset` elemanını seç
            slice_end_col = min(trapezoid_height + row_offset, row_length)
            series_data.extend(binomial_triangle_data[row_index][:slice_end_col])
            selected_indices_info.append({
                "row_index": row_index,
                "slice_start_col": 0,
                "slice_end_col": slice_end_col
            ...
            # Sağ hizalı yamuk: Her satırın son `trapezoid_height + row_offset` elemanını seç
            slice_start_col = max(0, row_length - (trapezoid_height + row_offset))
            series_data.extend(binomial_triangle_data[row_index][slice_start_col:])
            selected_indices_info.append({
                "row_index": row_index,
                "slice_start_col": slice_start_col,
                "slice_end_col": row_length
            ...
            # Merkez hizalı yamuk: Her satırın ortasından `trapezoid_height + row_offset` elemanını seç
            middle = row_length // 2
            half_width = (trapezoid_height + row_offset) // 2
            slice_start_col = max(0, middle - half_width)
            slice_end_col = min(row_length, middle + half_width + 1)
            series_data.extend(binomial_triangle_data[row_index][slice_start_col:slice_end_col])
            selected_indices_info.append({
                "row_index": row_index,
                "slice_start_col": slice_start_col,
